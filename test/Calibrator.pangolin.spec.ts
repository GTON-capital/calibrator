  ...
  describe("#calibrate", async () => {
    it("matches estimates", async () => {
      ...
      await expect(
        ...
        .withArgs(wallet.address, pair.address, "10000000000000000")
        .to.emit(weth, "Transfer")
        .withArgs(router.address, pair.address, "20000000000000000")
        .to.emit(pair, "Transfer")
        .withArgs(ZERO_ADDR, ZERO_ADDR, "1000")
        .to.emit(pair, "Transfer")
        .withArgs(ZERO_ADDR, wallet.address, "14142135623729950")
        .to.emit(pair, "Sync")
        .withArgs("20000000000000000", "10000000000000000")
        .to.emit(pair, "Mint")
        .withArgs(router.address, "20000000000000000", "10000000000000000")
      ...
      let reserveGTONEstimated = estimates[0] //"9853463255519303"
      let reserveTokenEstimated = estimates[1] //"20000000000000000"
      let amountGTONEstimated = estimates[2] //"146536744480697"
      let liquidityEstimated = estimates[3] //"13038005020143540"
      console.log(
      ...
      await expect(
        ...
        .to.emit(pair, "Sync")
        .withArgs("20000000000000000", "9853463255519303")
      ...
      expect(reserveGTONEstimated).to.eq("9853463255519303")
      expect(reserveTokenEstimated).to.eq("20000000000000000")
    })
  })
})
