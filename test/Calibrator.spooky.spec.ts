...
import { SpookyPair } from "../typechain/SpookyPair"
import { SpookyFactory } from "../typechain/SpookyFactory"
import { SpookyRouter02 } from "../typechain/SpookyRouter02"
...
describe("Spooky", () => {
  ...
  let factory: SpookyFactory
  let router: SpookyRouter02
  let pair: SpookyPair
  ...
  describe("#calibrate", async () => {
    it("matches estimates", async () => {
      ...
      const factoryFactory = await ethers.getContractFactory("SpookyFactory")
      const factory = await factoryFactory.deploy(wallet.address) as SpookyFactory

      await factory.setFeeTo(other.address)

      const routerFactory = await ethers.getContractFactory("SpookyRouter02")
      const router = await routerFactory.deploy(factory.address,weth.address) as SpookyRouter02

      const pairFactory = await ethers.getContractFactory("SpookyPair")
      ...
      const pair = pairFactory.attach(pairAddress) as SpookyPair

      // https://ftmscan.com/tx/0x59ac4f44e7ad5d5e5a7a7d837e66d9e71298695a3a951ba0771c7573a1ccd045#eventlog
      let liquidityGTON = BigNumber.from("10000000000000000000")
      let liquidityWETH = BigNumber.from("136000000000000000000")
      ...
      await expect(router.addLiquidityETH(
        ...
        .to.emit(weth, "Transfer").withArgs(router.address, pair.address, "136000000000000000000")
        .to.emit(pair, "Transfer").withArgs(ZERO_ADDR, ZERO_ADDR, "1000")
        .to.emit(pair, "Transfer").withArgs(ZERO_ADDR, wallet.address, "36878177829171548240")
        .to.emit(pair, "Sync").withArgs("136000000000000000000","10000000000000000000")
        .to.emit(pair, "Mint").withArgs(router.address, "136000000000000000000","10000000000000000000")
      ...
      await pair.approve(calibrator.address, "35878177829171549240")
      ...
      // 0x0000000000000000000000000000000000000000000000075f610f70ed200000
      // 0x0000000000000000000000000000000000000000000000008ac5c65f600efac2
      ...
      await expect(calibrator.calibrate(
        ...
        .to.emit(pair, "Sync").withArgs("136000000000000000000","9999616660581841602")
      ...
      expect(reserveGTONEstimated).to.eq("9999616660581841602")
      expect(reserveTokenEstimated).to.eq("136000000000000000000")
    })
  })
})
