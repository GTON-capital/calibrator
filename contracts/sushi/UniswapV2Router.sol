...
// File: contracts/uniswapv2/interfaces/ISushiPair.sol
...
// File: contracts/uniswapv2/libraries/SushiLibrary.sol
...
library SushiLibrary {
    ...
    function pairFor(address factory, address tokenA, address tokenB) internal pure returns (address pair) {
        ...
        pair = address(uint(keccak256(abi.encodePacked(
                ...
                hex'2afca130f6dee603872bc88397cb1ef20ac11dca2f145898c40f920c173c1876' // init code hash
            ))));
    }
...
// File: contracts/uniswapv2/interfaces/ISushiRouter01.sol
...
// File: contracts/uniswapv2/interfaces/ISushiRouter02.sol
...
// File: contracts/uniswapv2/SushiRouter02.sol
...
contract SushiRouter02 is ISushiRouter02 {
    using SafeMathUniswap for uint;
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0,) = SushiLibrary.sortTokens(input, output);
            ISushiPair pair = ISushiPair(SushiLibrary.pairFor(factory, input, output));
            uint amountInput;
            uint amountOutput;
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        TransferHelper.safeTransferFrom(
            path[0], msg.sender, SushiLibrary.pairFor(factory, path[0], path[1]), amountIn
        );
        uint balanceBefore = IERC20Uniswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Uniswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
            'SushiRouter: INSUFFICIENT_OUTPUT_AMOUNT'
        );
    }
    function swapExactETHForTokensSupportingFeeOnTransferTokens(
        uint amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        assert(IWETH(WETH).transfer(SushiLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint balanceBefore = IERC20Uniswap(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Uniswap(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        require(amountOut >= amountOutMin, 'SushiRouter: INSUFFICIENT_OUTPUT_AMOUNT');
