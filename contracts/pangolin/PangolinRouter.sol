...
// File hardhat/console.sol@v2.0.6
...
library PangolinLibrary {
            ...
            uint256(
                keccak256(
                    abi.encodePacked(
                        ...
                        hex"7feaa9ea5232acb6a7eb4f7bc814d433b2933bfab872aecd8181202c80c9a742" // init code hash
                    )
                )
            )
        );
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IPangolinPair(
    ...
    function quote(
        ...
        require(
            ...
            "PangolinLibrary: INSUFFICIENT_LIQUIDITY"
        );
...
contract PangolinRouter is IPangolinRouter {
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                require(
                    ...
                    "PangolinRouter: INSUFFICIENT_B_AMOUNT"
                );
                ...
                require(
                    ...
                    "PangolinRouter: INSUFFICIENT_A_AMOUNT"
                );
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? PangolinLibrary.pairFor(factory, output, path[i + 2])
                : _to;
        ...
        returns (uint256[] memory amounts)
    {
        ...
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            PangolinLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
        ...
        returns (uint256[] memory amounts)
    {
        ...
        require(
            ...
            "PangolinRouter: EXCESSIVE_INPUT_AMOUNT"
        );
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
    ...
    function _swapSupportingFeeOnTransferTokens(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            IPangolinPair pair = IPangolinPair(
                PangolinLibrary.pairFor(factory, input, output)
            );
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = PangolinLibrary.getAmountOut(
                    amountInput,
                    reserveInput,
                    reserveOutput
                );
